...
func configKeyCatalog() []inferctl.ConfigKeyDef {
	return []inferctl.ConfigKeyDef{
		...
		keyDef("backends.<name>.credential.source", "enum string", true, nil, "Typed credential reference source. literal stores a local value; environment names a process environment variable.", []string{config.CredentialSourceLiteral, config.CredentialSourceEnvironment}, config.CredentialSourceLiteral),
